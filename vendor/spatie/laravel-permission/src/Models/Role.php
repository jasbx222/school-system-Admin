...
class Role extends Model implements RoleContract
{
    ...
    public static function findOrCreate(string $name, ?string $guardName = null): RoleContract
    {
        ...
        if (! $role) {
            return static::create(['name' => $name, 'guard_name' => $guardName] + (app(PermissionRegistrar::class)->teams ? [app(PermissionRegistrar::class)->teamsKey => getPermissionsTeamId()] : []));
        }
    ...
    public function hasPermissionTo($permission, ?string $guardName = null): bool
    {
        ...
        $permissionId = (is_object($permission) && method_exists($permission, 'getKey'))
            ? $permission->getKey()
            : ((is_object($permission) && property_exists($permission, 'id'))
                ? $permission->id
                : (is_array($permission) && isset($permission['id'])
                    ? $permission['id']
                    : (int)$permission));

        return $this->loadMissing('permissions')->permissions
            ->contains('id', $permissionId);
    }
}
